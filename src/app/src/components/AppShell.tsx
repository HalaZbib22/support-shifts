...
export function AppShell({ children }: { children: React.ReactNode }) {
  const { loading, user, profile, domainError, signIn, signOutUser } = useAuth();
  ...
  if (!user) return <SignIn onSignIn={signIn} domainError={domainError} />;
...
function SignIn({ onSignIn, domainError }: { onSignIn: () => Promise<void>; domainError: string | null }) {
  ...
  return (
    <Box sx={{ minHeight: "100vh", display: "grid", placeItems: "center", position: "relative", p: 2 }}>
      ...
      <Blueprint sx={{ width: 380, maxWidth: "100%", p: "28px 28px 24px" }}>
        ...
        {domainError && (
          <Box sx={{ border: `1px solid ${T.err}`, color: T.errDeep, p: "10px 12px", fontSize: 13, mb: 2 }}>{domainError}</Box>
        )}
        {err && (
