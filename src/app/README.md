...
**Domain restriction.** Sign-in is limited to one Google Workspace domain, enforced in two places:

- `firestore.rules` — `allowedDomain()` near the top returns `koein.com`. Every read and write requires a verified email on that domain, so an outside account can't touch the database whatever the client does. Change the domain there and redeploy the rules.
- `NEXT_PUBLIC_ALLOWED_DOMAIN=koein.com` — filters the Google account picker and shows a clear "limited to @koein.com accounts" message instead of letting someone sign in and hit permission errors they can't interpret.

The rules are the enforcement; the env var is the courtesy. Set both to the same domain.

**Careful:** the first admin has to be on that domain too. If you set up the project with a personal account, sign in with your work address first, then move `isAdmin` to that document before deploying these rules — otherwise you lock yourself out.
